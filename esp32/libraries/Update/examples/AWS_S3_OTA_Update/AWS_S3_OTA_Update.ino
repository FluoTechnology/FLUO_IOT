...
// OTA Logic 
void execOTA() {
  FluoTube.debugln("Connecting to: " + String(host));
  // Connect to S3
  if (client.connect(host.c_str(), port)) {
    // Connection Succeed.
    // Fecthing the bin
    FluoTube.debugln("Fetching Bin: " + String(bin));
    ...
    //    FluoTube.debug(String("GET ") + bin + " HTTP/1.1\r\n" +
    //                 "Host: " + host + "\r\n" +
    //                 "Cache-Control: no-cache\r\n" +
    //                 "Connection: close\r\n\r\n");
    ...
    */
    while (client.available()) {
      ...
      if (line.startsWith("HTTP/1.1")) {
        if (line.indexOf("200") < 0) {
          FluoTube.debugln("Got a non 200 status code from server. Exiting OTA Update.");
          break;
        }
      }
      ...
      if (line.startsWith("Content-Length: ")) {
        ...
        FluoTube.debugln("Got " + String(contentLength) + " bytes from server");
      }

      // Next, the content type
      if (line.startsWith("Content-Type: ")) {
        String contentType = getHeaderValue(line, "Content-Type: ");
        FluoTube.debugln("Got " + String(contentType) + " payload.");
    ...
    FluoTube.debugln("Connection to " + String(host) + " failed. Please check your setup");
    // retry??
    // execOTA();
  }

  // Check what is the contentLength and if content type is `application/octet-stream`
  FluoTube.debugln("contentLength : " + String(contentLength) + ", isValidContentType : " + String(isValidContentType));
  ...
  if (contentLength && isValidContentType) {
    ...
    // If yes, begin
    if (canBegin) {
      FluoTube.debugln("Begin OTA. This may take 2 - 5 mins to complete. Things might be quite for a while.. Patience!");
      ...
      if (written == contentLength) {
        ...
        FluoTube.debugln("Written only : " + String(written) + "/" + String(contentLength) + ". Retry?" );
        // retry??
        // execOTA();
      }
    ...
    FluoTube.debugln("There was no content in the response");
    client.flush();
  }
}
...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    FluoTube.debug("."); // Keep the serial monitor lit!
    delay(500);
  }

  // Connection Succeed
  FluoTube.debugln("");
  FluoTube.debugln("Connected to " + String(SSID));
