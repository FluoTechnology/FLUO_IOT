...
void ScanForSlave() {
  ...
  if (scanResults == 0) {
    ...
    FluoTube.debug("Found "); 
    FluoTube.debug( String(scanResults) ); 
    FluoTube.debugln(" devices ");
  ...
  if (SlaveCnt > 0) {
    FluoTube.debug( String(SlaveCnt) ); 
    FluoTube.debugln(" Slave(s) found, processing.. ");
  } else {
    FluoTube.debugln("No Slave Found, trying again.");
  }
...
void manageSlave() {
  if (SlaveCnt > 0) {
    for (int i = 0; i < SlaveCnt; i++) {
      ...
      FluoTube.debug("Processing: ");

      for (int ii = 0; ii < 6; ++ii ) {

        FluoTube.debug( String( (uint8_t) slaves[i].peer_addr[ii], HEX ) );

        if (ii != 5) FluoTube.debug(":");
      }

      FluoTube.debug(" Status: ");
      ...
      else 
      {
        ...
        if (addStatus == ESP_OK) {
          ...
          FluoTube.debugln("Add Peer - Invalid Argument");
...
void setup() {
  ...
  WiFi.mode(WIFI_STA);
  FluoTube.debugln("ESPNow/Multi-Slave/Master Example");
