...
{
    FluoTube.setup();
    FluoTube.pinMode(13, OUTPUT);      // set the LED pin mode
...
void loop(){
  ...
  if (client) {                             // if you get a client,
    FluoTube.debugln("New Client");           // print a message out the serial port
    ...
    while (client.connected()) {            // loop while the client's connected
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (currentLine.endsWith("GET /H")) {
          FluoTube.digitalWrite(13, HIGH);               // GET /H turns the LED on
        }
        if (currentLine.endsWith("GET /L")) {
          FluoTube.digitalWrite(13, LOW);                // GET /L turns the LED off
        }
      }
    }
    // close the connection:
    client.stop();
    FluoTube.debugln("Client Disconnected");
  }
}
