...
#define RST_TIME 500 // 50 microseconds normal value / 500 only if you a porkaround for V01
    ...
    switch (_state) {
        case AVRISP_STATE_IDLE: {
           ...
           if (_client) {
                ...
                IPAddress lip;
                lip = _client.remoteIP();
                log_d("client connect");
                _client.setTimeout(500); // for getch() // ho esagerato ma va' e non lo tocco piu'
...
inline void ESP8266AVRISP::_reject_incoming(void) {
    while ( _server.available() ) _server.available().stop();
}
...
void ESP8266AVRISP::start_pmode() {
    ...
    SPIH.begin(14,12,13);
    SPIH.setFrequency(_spi_freq);
    SPIH.setHwCs(false);

    // try to sync the bus
    SPIH.transfer(0x00);
    digitalWrite(_reset_pin, _resetLevel(false));
    delayMicroseconds(RST_TIME);
...
int ESP8266AVRISP::addr_page(int addr) {
    ...
    log_d("unknown page size: %d", param.pagesize);
    return addr;
}
